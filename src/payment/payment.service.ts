...
export class PaymentService {
  ...
  async createSubscription(
    ...
    try {
      ...
      try {
        ...
        throw new BadRequestException('Price ID inválido ou não encontrado no Stripe');
      }
  ...
  async createSubscriptionWithTestPaymentMethod(
    ...
    try {
      ...
      if ((stripeSubscription as any).latest_invoice?.payment_intent) {
        const paymentIntent = (stripeSubscription as any).latest_invoice.payment_intent;
        console.log('🔗 Configurando return_url para PaymentIntent:', paymentIntent.id);
      }
      ...
      console.error('❌ Erro ao criar assinatura com método de pagamento:', error);
      throw new BadRequestException('Erro ao processar assinatura');
    }
  }
  ...
  async getSubscription(userId: string): Promise<SubscriptionResponseDto | null> {
    ...
    return subscription ? plainToInstance(SubscriptionResponseDto, subscription) : null;
  }
